...
export class ContactFormComponent implements OnInit {
  ...
  async onSubmit() {
    ...
    const contact = new Contact({
      ...
      dateDeNaissance: formatDate(
        this.contactForm?.value.date,
        'yyyy-MM-dd',
        'en-US'
      ),
    ...
    this.contactId
      ...
      : await this.contactService.postContact(contact);
  ...
  async ngOnInit() {
    ...
    if (this.contactId) {
      ...
      addresses = this.contact?.addresses
        ? this.contact.addresses.map((address) => this.creeAddresse(address))
        : [this.creeAddresse()];
    }
    this.contactForm = this.formBuilder.group({
      nom: this.formBuilder.control(!this.contactId ? '' : this.contact?.nom),
      prenom: this.formBuilder.control(
        !this.contactId ? '' : this.contact?.prenom
      ),
      date: this.formBuilder.control(
        !this.contactId ? '' : new Date(this.contact?.dateDeNaissance ?? '')
      ),
      addresses: this.formBuilder.array(
        !this.contactId ? [this.creeAddresse()] : addresses
      ),
    });
